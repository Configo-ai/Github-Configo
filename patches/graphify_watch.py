...
def _resolve_out(watch_path: Path) -> Path:
    """Return the output directory.
    Respects GRAPHIFY_OUT env var so output can be redirected to a shared
    shared workspace graph directory (e.g. Github-Configo/graphify) without changing cwd.
    Falls back to the classic <watch_path>/graphify-out.
    """
    env = os.environ.get("GRAPHIFY_OUT")
    if env:
        return Path(env).expanduser().resolve()
    return watch_path / "graphify-out"
...
def _notify_only(watch_path: Path) -> None:
    ...
    flag = _resolve_out(watch_path) / "needs_update"
    ...
    class Handler(FileSystemEventHandler):
        def on_any_event(self, event):
            ...
            if any(part.startswith(".") for part in path.parts):
                return
            out_dir = _resolve_out(Path(".")).name
            if "graphify-out" in path.parts or out_dir in path.parts:
                return
